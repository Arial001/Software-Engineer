...
public class InteractScriptBodyP3 : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (selectionManager == null || raycastVisualizer == null || VerifP3 == null)
        {
        ...
        if (button2 != null && button2.wasPressedThisFrame)
        {
            ...
            if (objectInFront != null)
            {
                ...
                {
                    for (int i = 0; i < VerifP3.PlayerBodyP3.Count; i++)
                    {
                        GameObject listObject = VerifP3.PlayerBodyP3[i];

                        if (listObject.name == A)
                        {
                            VerifP3.PlayerBodyP3[i].gameObject.SetActive(true);
                            Debug.Log($"Objek '{A}' ditemukan pada indeks {i}");
                            VerifJumlahPemain.ChooseBodyP4();
                            return;
                        }
                        VerifJumlahPemain.ChooseBodyP4();
                    }
